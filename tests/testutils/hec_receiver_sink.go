...
// Build will create, configure, and start an HECReceiver with associated log and metric sinks.
...
func (hec *HECReceiverSink) Start() error {
	...
	if err := (*hec.logsReceiver).Start(context.Background(), hec.Host); err != nil {
		return err
	}
	return (*hec.metricsReceiver).Start(context.Background(), hec.Host)
}
...
func (hec *HECReceiverSink) Shutdown() error {
	...
	if err := (*hec.logsReceiver).Shutdown(context.Background()); err != nil {
		return err
	}
	return (*hec.metricsReceiver).Shutdown(context.Background())
}
